...
项目也提供了 `fly.toml`。如果你更想用 Fly.io：

```bash
fly launch --no-deploy
fly secrets set DEEPSEEK_API_KEY="你的DeepSeekKey" APP_PASSWORD="设置一个强密码"
fly deploy
```

Fly.io 会读取 `fly.toml` 和 `Dockerfile`，并使用 `/healthz` 做健康检查。
如果 Fly.io 提示 app 名称冲突，换一个唯一名称，或编辑 `fly.toml` 里的 `app`。
